...
class CertificateClaim(BaseModel):
    ...
    code: constr(strict=True, max_length=50, min_length=0) = Field(
        ...,
        description="[Tarneahela sertifikaadi väite kood](#operation/Certificates_List)",
    )
    number: Optional[constr(strict=True, max_length=50)] = Field(
        None, description="Sertifikaadi number"
    )
    __properties = ["code", "number"]
    ...
    @classmethod
    def from_json(cls, json_str: str) -> CertificateClaim:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> CertificateClaim:
        """Create an instance of CertificateClaim from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return CertificateClaim.parse_obj(obj)

        _obj = CertificateClaim.parse_obj(
