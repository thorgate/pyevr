...
class ForestNotice(HoldingBase):
    """
    ForestNotice
    """
    ...
    number: Optional[constr(strict=True, max_length=500)] = Field(
        None, description="Metsateatise number"
    )
    __properties = [
        "type",
        "cadaster",
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ForestNotice:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> ForestNotice:
        """Create an instance of ForestNotice from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return ForestNotice.parse_obj(obj)

        _obj = ForestNotice.parse_obj(
            {
                "type": obj.get("type"),
