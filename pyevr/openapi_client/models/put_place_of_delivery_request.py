...
class PutPlaceOfDeliveryRequest(BaseModel):
    """
    PutPlaceOfDeliveryRequest
    """

    name: constr(strict=True, min_length=0) = Field(..., description="Nimi")
    ...
    is_active: StrictBool = Field(..., alias="isActive", description="Kas on aktiivne")
    is_public: StrictBool = Field(
        ...,
        alias="isPublic",
        description="Kas on avalik (avalikke ladusid näevad ka teised asutused)",
    )
    ...
    waybill_authorizations: Optional[conlist(WaybillAuthorization)] = Field(
        None,
        alias="waybillAuthorizations",
        description="Volitatud mõõtjad ja vaatlejad",
    )
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PutPlaceOfDeliveryRequest:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PutPlaceOfDeliveryRequest:
        """Create an instance of PutPlaceOfDeliveryRequest from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PutPlaceOfDeliveryRequest.parse_obj(obj)

        _obj = PutPlaceOfDeliveryRequest.parse_obj(
            {
                "name": obj.get("name"),
