...
class Waybill(BaseModel):
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Optional[Union[ForestWaybill, SawnWaybill]]:
    ...
    ) -> Optional[Union[ForestWaybill, SawnWaybill]]:
        """Create an instance of Waybill from a dict"""
        # look up the object type based on discriminator mapping
        object_type = cls.get_discriminator_value(obj)
        if object_type == "ForestWaybill":
            return import_module(
                "pyevr.openapi_client.models.forest_waybill"
            ).ForestWaybill.from_dict(obj)
        if object_type == "SawnWaybill":
            return import_module(
                "pyevr.openapi_client.models.sawn_waybill"
            ).SawnWaybill.from_dict(obj)

        raise ValueError(
            "Waybill failed to lookup discriminator value from "
            + json.dumps(obj)
